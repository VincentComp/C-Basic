...
int main()
{
    ...
    /* 
    1)cin >> inputNumber
    2)check (type of inputNumber == int)
    3)check (inputNumber == +ve)
    4)if false --> clear the cin --> repeat
    ...
    */   
    ...
    //===========Part II==================
    /* 
    1) Set num = inputNumber(input value)
    2) divide inputNumber by 10
       for each successful division, numOfDigits ++
       until input inputNumber <= 0
    3) numOfDigits of inputNumber is got
    */   
    ...
    //===========Part III==================
    /* 
    1) when numofDigits = n,
        no. of substring = 1+2+3+4+...+n
    2)Total number of substring = n(n+1)/2
    3)numOfTotalSubstr is got
    4)Output: inputNumber is divisible by its substrings:
    */   
    ...
    */
    ...
    // ======B Part IV =======
    /*
    1) set totalCount(substring|NumInput = T --> totalCount++  )
    2)(int i = numOfDigit ; i>0; i--)
        longest_substring = inputNumber % (10^i);   e.g. 1234%10^4 = 1234 --> 1234%10^1 = 4

        3) set j = (j=0,j<i,j++) 
        substring = longest_substring / (10^j) e.g. 1234/ 10^0 = 1234 ---> 1234/10^3 = 1 

        4) test whether substring|input Number
            if true --> print & totalCount++
               false --> next substring


    <reminders>
    (1)pow(a,b) give float
    (2) a % 10^n give last n digit
    (3) a / 10^n give first(total-n) digit
    */
    ...
    { int longest_substring = inputNumber % static_cast<int>(pow(10,i));
    ...
    int substring;
    for (int j = 0; j < i; j ++ )
        {substring = longest_substring / static_cast<int> (pow(10,j)) ;
