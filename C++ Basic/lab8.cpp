...
void stock_checking(const Fruit* all_fruits){
    for(const Fruit* temp = all_fruits; temp != nullptr; temp = temp ->next)
    cout << temp->name << ": "<<temp -> total_count << endl;
}
...
void stock_updating(Fruit* all_fruits, int fruit_id_to_sell, int number_to_sell){
    Fruit* temp = all_fruits;
    for(int position = 0;position < fruit_id_to_sell && temp->next != nullptr; temp = temp ->next, position ++);
    temp -> total_count -= number_to_sell;
    temp -> sold_count += number_to_sell;
...
float selling_amount_calculating(const Fruit* all_fruits){
    int amount = 0;
    for(const Fruit* temp = all_fruits; temp != nullptr; temp = temp ->next)
    amount +=(temp -> unit_price)*( temp -> sold_count); 
    return amount;
}
...
int main() {
    ...
    all_fruits = &fruit1; //all_fruits = head ---> fruit 1
    Fruit fruit2 = {1, "Banana", 3, 30, 0, nullptr};
    warehousing(&fruit2, all_fruits); //all_fruits = head ---> fruit 1 --> fruiut 2   
